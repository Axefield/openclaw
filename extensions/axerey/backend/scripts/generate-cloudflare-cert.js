...
class CloudflareCertGenerator {
    ...
    constructor() {
        ...
        this.certDir = (0, path_1.join)(process.cwd(), 'certs');
    ...
    async getZoneId() {
        ...
        try {
            const response = await (0, node_fetch_1.default)('https://api.cloudflare.com/client/v4/zones', {
                headers: {
    ...
    async generateCertificate() {
        ...
        try {
            ...
            const response = await (0, node_fetch_1.default)('https://api.cloudflare.com/client/v4/certificates', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${this.apiToken}`,
                    'Content-Type': 'application/json'
    ...
    async saveCertificates(cert, key) {
        ...
        if (!(0, fs_1.existsSync)(this.certDir)) {
            (0, fs_1.mkdirSync)(this.certDir, { recursive: true });
            console.log(`📁 Created directory: ${this.certDir}`);
        }
        const certPath = (0, path_1.join)(this.certDir, 'cert.pem');
        const keyPath = (0, path_1.join)(this.certDir, 'key.pem');
        // Save certificate
        (0, fs_1.writeFileSync)(certPath, cert, 'utf8');
        console.log(`💾 Saved certificate: ${certPath}`);
        // Save private key
        (0, fs_1.writeFileSync)(keyPath, key, 'utf8');
...
exports.default = CloudflareCertGenerator;
